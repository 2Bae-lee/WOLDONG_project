...
const sectionsToPayload = (name: string, sections: ProfileSection[]): ChildProfileUpdatePayload => {
...
export default function ParentChildProfile() {
    ...
    useEffect(() => {
        ...
        const loadProfile = async () => {
            ...
            try {
                ...
                if (!nextChildId) throw new Error('저장할 아동 프로필을 찾지 못했어요.');
                ...
                setSections(sectionsFromDetail(response.data));
                if (!profileImage && response.data.profile_image_url) {
                    setProfileImage(response.data.profile_image_url);
                }
            } catch (error) {
                if (!active) return;
                setProfileError(error instanceof Error ? error.message : '저장된 아동 프로필을 불러오지 못했어요.');
            } finally {
    ...
    const handleComplete = async () => {
        ...
        try {
            ...
            await updateChildProfile(nextChildId, {
                ...sectionsToPayload(childName, sections),
            ...
            setProfileError(error instanceof Error ? error.message : '아동 프로필 저장에 실패했어요.');
        } finally {
